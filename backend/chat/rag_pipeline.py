...
def detect_intent(question: str) -> str:
    """
    Detect query intent to guide retrieval and routing strategy.

    Returns:
      "hr_write" - employee update/CRUD -> MCP tool (MongoDB write)
      "hr_read"  - employee data query  -> MCP tool (MongoDB read)
      "summary"  - overview/broad       -> RAG with aggregation
      "entity"   - doc entity queries   -> RAG with entity filter
      "default"  - everything else      -> RAG standard pipeline
    """
    from backend.mcp.tool_executor import detect_hr_intent

    # Check MCP intents first — most specific
    hr_intent = detect_hr_intent(question)
    if hr_intent in ("hr_read", "hr_write"):
        return hr_intent

    q = question.lower()

    summary_signals = [
        "overview", "give me a summary", "summarize",
        "tell me everything", "everything about",
        "give me an overview", "overall performance",
        "in detail", "all about", "discuss in detail",
        "how is the company doing", "how is the team doing",
        "what's happening in", "whats happening in",
    ]
    # Note: "what is X" and "explain X" removed from summary signals
    # These are simple factual questions → default pipeline handles them fine
    # Summary pipeline is only for broad aggregation queries

    entity_signals = [
        "who is", "who are", "person", "details of",
...
def get_chat_response(
    ...
    # ── Step 3b: MCP routing — HR read/write goes to MongoDB via tools ────
    # If intent is hr_read or hr_write, bypass RAG entirely
    # MCP tools handle employee data; RAG handles policy documents
    if intent in ("hr_read", "hr_write"):
        # Only HR and C_LEVEL can access employee data tools
        if role not in ("HR", "C_LEVEL"):
            logger.warning(f"[MCP] Blocked: role={role} tried hr tool")
            return {
                "answer": "Employee records are only accessible to HR and C-Level staff.",
                "sources": [],
            }
        logger.info(f"[MCP] Routing to MCP agent: intent={intent}")
        ...
        logger.info(f"[TIMING] MCP agent: {time.perf_counter() - t0:.3f}s | tool={result.get('tool_used')}")
        return result
